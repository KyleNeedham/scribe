describe 'Translator Service', ->
  ...
  describe 'retrieving', ->
    ...
    it 'should return the translation key if unable to retrieve a translation', ->
      ...
      expect(translator.get('replace.not_found', {bar: 'great'})).toEqual 'replace.not_found'

  describe 'replacing', ->
    it 'should replace a singular place-holder', ->
      expect(translator.get('replace.singular', bar: 'great')).toEqual 'foo is great'

    it 'should replace multiple place-holders', ->
      expect(translator.get('replace.multiple', bar: 'great', foo: 'greator')).toEqual 'foo is great but bar is greator'

    it 'should replace multiple place-holders of different lengths correcty', ->
      expect(translator.get('replace.complex', abc: 'great', abcdef: 'greator')).toEqual 'foo is great but bar is greator'

    it 'should replace a place-holder multiple times', ->
      expect(translator.get('replace.same', {bar: 'great'})).toEqual 'foo is great and bar is also great'

    it 'should replace the same place-holder multiple times with different data each time using an array', ->
      expect(translator.get('replace.same-array', {foo: [1, 2, 3, 4, 5]})).toEqual 'lets count to 5, 1 2 3 4 5'
